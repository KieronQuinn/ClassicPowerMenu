...
public class ServiceListing {
    ...
    public void reload() {
        loadEnabledServices();
        mServices.clear();
        final int user = ActivityManager_getCurrentUser();
        ...
        List<ResolveInfo> installedServices = pmWrapper.queryIntentServices(
                new Intent(mIntentAction), flags);

        for (ResolveInfo resolveInfo : installedServices) {
            ServiceInfo info = resolveInfo.serviceInfo;

            if (!mPermission.equals(info.permission)) {
                Log.w(mTag, "Skipping " + mNoun + " service "
