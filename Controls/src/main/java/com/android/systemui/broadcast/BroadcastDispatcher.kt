...
open class BroadcastDispatcher constructor (
    ...
    fun initialize() {
        ...
        registerReceiver(this, IntentFilter(Intent_ACTION_USER_SWITCHED), null, UserHandle_ALL)
    }

    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action == Intent_ACTION_USER_SWITCHED) {
            val user = intent.getIntExtra(Intent_EXTRA_USER_HANDLE, UserHandle_USER_NULL)
    ...
    private val handler = object : Handler(bgLooper) {
        var currentUser = UserHandle_USER_SYSTEM
        ...
        override fun handleMessage(msg: Message) {
            when (msg.what) {
                MSG_ADD_RECEIVER -> {
                    ...
                    val userId = if (data.user.identifier == UserHandle_USER_CURRENT) {
                        currentUser
                    } else {
                        data.user.identifier
                    }
                    if (userId < UserHandle_USER_ALL) {
                ...
                MSG_SET_STARTING_USER -> {
                    currentUser = ActivityManager_getCurrentUser()
                }
