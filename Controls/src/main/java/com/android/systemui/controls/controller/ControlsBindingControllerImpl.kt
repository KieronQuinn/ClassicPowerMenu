...
import com.android.systemui.util.extensions.*
import java.util.concurrent.atomic.AtomicBoolean

@VisibleForTesting
open class ControlsBindingControllerImpl constructor(
    private val context: Context,
    private val backgroundExecutor: DelayableExecutor,
    ...
    private var currentUser = UserHandle_of(ActivityManager_getCurrentUser())
    ...
    internal open fun createProviderManager(component: ComponentName):
        ...
        return ControlsProviderLifecycleManager(
                ...
                currentUser,
                component,
                this::bindServiceInternal,
                this::unbindServiceInternal
        )
    }

    private fun bindServiceInternal(context: Context, intent: Intent, serviceConnection: ServiceConnection, flags: Int){
        serviceRunner.bindService(context, intent, serviceConnection, flags)
    }

    private fun unbindServiceInternal(context: Context, serviceConnection: ServiceConnection){
        serviceRunner.unbindService(context, serviceConnection)
    }
    ...
    private abstract inner class CallbackRunnable(val token: IBinder) : Runnable {
        ...
        override fun run() {
            ...
            /*val providerInterface = IControlTileToken.Stub.asInterface(token)
            if (providerInterface.id != provider.controlProviderId) {
                Log.e(TAG, "Provider for token:$token does not exist anymore ${providerInterface.id} vs ${provider.controlProviderId}")
                return
            }*/
    ...
    private inner class OnActionResponseRunnable(
        ...
        @ResponseResult val response: Int
    ) : CallbackRunnable(token) {
        override fun doRun() {
            provider?.let {
