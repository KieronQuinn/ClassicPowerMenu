...
class ControlsControllerImpl constructor (
    ...
    companion object {
        ...
        internal const val CONTROLS_AVAILABLE = "controls_enabled"
        ...
        private fun isAvailable(cr: ContentResolver) = Settings.Secure.getInt(cr, CONTROLS_AVAILABLE, DEFAULT_ENABLED) != 0
    }
    ...
    override var available = isAvailable(contentResolver)
        private set
    ...
    private fun setValuesForUser(newUser: UserHandle) {
        ...
        available = isAvailable(contentResolver)
    ...
    private val userSwitchReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            if (intent.action == Intent_ACTION_USER_SWITCHED) {
                userChanging = true
                val newUser =
                        UserHandle_of(intent.getIntExtra(Intent_EXTRA_USER_HANDLE, sendingUserId))
    ...
    internal val restoreFinishedReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            val user = intent.getIntExtra(Intent_EXTRA_USER_ID, UserHandle_USER_NULL)
    ...
    init {
        ...
        broadcastDispatcher.registerReceiver(
                userSwitchReceiver,
                IntentFilter(Intent_ACTION_USER_SWITCHED),
                executor,
                UserHandle_ALL
        )
        ...
        contentResolver.registerContentObserver(URI, false, settingObserver)
...
class UserStructure(context: Context, user: UserHandle) {
    val userContext = context //context.createContextAsUser(user, 0)

    val file = Environment_buildPath(
            userContext.filesDir,
            ControlsFavoritePersistenceWrapper.FILE_NAME
    )

    val auxiliaryFile = Environment_buildPath(
            userContext.filesDir,
