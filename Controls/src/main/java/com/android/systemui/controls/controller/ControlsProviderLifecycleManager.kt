...
import com.android.systemui.util.extensions.ControlsProviderService_CALLBACK_BUNDLE
import java.util.*
...
class ControlsProviderLifecycleManager(
    ...
    val bindService: (Context, Intent, ServiceConnection, Int) -> Unit,
    val unbindService: (Context, ServiceConnection) -> Unit
    ...
    companion object {
        ...
        private val BIND_FLAGS = Context.BIND_AUTO_CREATE or Context.BIND_NOT_PERCEPTIBLE
    }
        ...
        executor.execute {
            ...
            if (bind) {
                if (bindTryCount != MAX_BIND_RETRIES) {
                    ...
                    bindTryCount++
                    try {
                        //context.bindService(intent, serviceConnection, BIND_FLAGS)
                        bindService(context, intent, serviceConnection, BIND_FLAGS)
    ...
    private fun handlePendingServiceMethods() {
        ...
        Log.d(TAG, "handle queue ${queue.size} items to run")
        queue.forEach {
            it.run()
        }
    }
    ...
    private fun invokeOrQueue(sm: ServiceMethod) {
        wrapper?.run {
            Log.d(TAG, "invokeOrQueue run")
            sm.run()
        } ?: run {
            Log.d(TAG, "invokeOrQueue queue")
    ...
    inner class Load(val subscriber: IControlsSubscriber.Stub) : ServiceMethod() {
        override fun callWrapper(): Boolean {
            if (DEBUG) {
                Log.d(TAG, "load $componentName $wrapper")
            }
            val result = wrapper?.load(subscriber) ?: false
