...
class AppAdapter(
     ...
     */
    class Holder(view: View, val favRenderer: FavoritesRenderer) : RecyclerView.ViewHolder(view), KoinComponent {

        private val monet by inject<MonetColorProvider>()

        private val icon: ImageView = itemView.requireViewById(android.R.id.icon)
        private val title: TextView = itemView.requireViewById(android.R.id.title)
        ...
        fun bindData(data: ControlsServiceInfo) {
            ...
            monet.applyMonetToView(title, true)
