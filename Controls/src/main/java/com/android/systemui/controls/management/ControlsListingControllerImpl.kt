...
import com.android.systemui.util.extensions.ActivityManager_getCurrentUser
import com.android.systemui.util.extensions.componentName
import com.android.systemui.util.extensions.identifier
...
class ControlsListingControllerImpl @VisibleForTesting constructor(
    ...
    override var currentUserId = ActivityManager_getCurrentUser()
        private set

    private val serviceListingCallback = ServiceListing.Callback {
        val newServices = it.toList()
        val newComponents =
            newServices.mapTo(mutableSetOf<ComponentName>(), { s -> s.componentName } )
    ...
    override fun changeUser(newUser: UserHandle) {
        ...
        backgroundExecutor.execute {
            if (userChangeInProgress.decrementAndGet() == 0) {
                ...
                val contextForUser = context //context.createContextAsUser(newUser, 0)
