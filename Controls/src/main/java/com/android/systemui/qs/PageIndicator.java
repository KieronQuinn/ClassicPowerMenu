...
import com.android.systemui.util.extensions.Extensions_AnimatedVectorDrawableKt;
import com.android.systemui.util.extensions.Extensions_ViewKt;
...
public class PageIndicator extends ViewGroup {
    ...
    private void setPosition(int position) {
        if (Extensions_ViewKt.isVisibleToUser(this) && Math.abs(mPosition - position) == 1) {
            animate(mPosition, position);
        } else {
            if (DEBUG) Log.d(TAG, "Skipping animation " + Extensions_ViewKt.isVisibleToUser(this) + " " + mPosition
                    + " " + position);
            setIndex(position >> 1);
        }
        mPosition = position;
    ...
    private void playAnimation(ImageView imageView, int res) {
        ...
        Extensions_AnimatedVectorDrawableKt.forceAnimationOnUI(avd);
        avd.start();
