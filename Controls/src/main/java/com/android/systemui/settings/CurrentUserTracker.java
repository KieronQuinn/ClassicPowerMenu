...
import static com.android.systemui.util.extensions.Extensions_ActivityManagerKt.ActivityManager_getCurrentUser;
...
public abstract class CurrentUserTracker {
    ...
    static class UserReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (Extensions_IntentKt.getIntent_ACTION_USER_SWITCHED().equals(intent.getAction())) {
                notifyUserSwitched(intent.getIntExtra(Extensions_IntentKt.getIntent_EXTRA_USER_HANDLE(), 0));
            }
        }
        ...
        private void addTracker(Consumer<Integer> callback) {
            ...
            if (!mReceiverRegistered) {
                mCurrentUserId = ActivityManager_getCurrentUser();
                IntentFilter filter = new IntentFilter(Extensions_IntentKt.getIntent_ACTION_USER_SWITCHED());
